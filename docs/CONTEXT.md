...
| Governance (`governance/`) | **Updated 2026-08-27.** The LangGraph workflow is on `main` via PR #6 — four agent nodes running in parallel in one superstep, a coordinator, `stub` mode, and `recommend()` as the whole public surface. Verified end-to-end against the real `trust_engine.evaluate()` since PR #7 landed: a 10/10 record yields `wilson_lower=0.7225`, `recommend()` returns HOLD with `has_dissent=True`. Prompt scaffolding is on `vc/prompts-and-cached-mode` (PR open) — versioned prompt files per agent, a deterministic evidence renderer, and a Pydantic + Gemini-schema boundary validating every model response. 121 tests, ruff clean, zero LLM calls anywhere | `cached` and `live` modes raise `NotImplementedError` naming their due dates rather than silently serving stub opinions | The Gemini client and cached replay (due 30 Aug); live mode with timeout/retry/fallback (due 3 Sept) |
...
> Governance row was re-verified by its owner on 2026-08-27 and is accurate as
